...
launch_sites = spacex_df.groupby(['Launch Site'], as_index=False).first()['Launch Site'].tolist()
...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        fig = px.pie(filtered_df['class'].value_counts(), values='class', names=['0','1'], 
...
def get_scatter_chart(entered_site,payload_slider_range):
    ...
    if entered_site == 'ALL':
        ...
        fig = px.scatter(filtered_df2, x='Payload Mass (kg)',y='class',color="Booster Version Category",
        title="Payload vs Success site "+entered_site)
        return fig
